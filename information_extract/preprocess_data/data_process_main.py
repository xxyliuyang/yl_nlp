...
from information_extract.preprocess_data.preprocessor import Preprocessor
...
def load_config():
    ...
    config = yaml.load(open("preprocess_data/build_data_config.yaml", "r"), Loader=yaml.FullLoader)
    return config
    ...
    for path, folds, files in os.walk(data_in_dir):
        for file_name in files:
            ...
            try :
                ...
                file_name2data[file_name] = [json.loads(line.strip()) for line in open(file_path, "r", encoding="utf-8")]
...
def get_processor():
    ...
    return preprocessor, get_tok2char_span_map
...
def add_char_and_token_span():
    ...
    for file_name, data in file_name2data.items():
        ...
        if "relation_list" in data[0]:  # train or valid data
            ...
            file_name2data[file_name] = data
    return rel_set, ent_set, error_statistics

def output_data(rel_set, ent_set):
...
if __name__ == '__main__':
    ...
    file_name2data = load_data()

    # 2.processor
    preprocessor, get_tok2char_span_map = get_processor()

    # 3.transform
    transform_format()

    # 4.添加span
    rel_set, ent_set, error_statistics = add_char_and_token_span()

    # 5.保存
    output_data(rel_set, ent_set)
